...
{                                                   // no guarantee all given pages are mapped
  ...
  for (pgit = 0; pgit < pgnum; pgit++)
  {
    ...
    // init_pte(pte, 1, frames[pgit].fpn, 0, 0, 0, 0); // Initialize PTE entry
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        // Update page table
        pte_set_swap(&caller->mm->pgd[victpgn], 0, free_fpn);

        // enlist_framephy_node(&caller->active_mswp->used_fp_list, free_fpn);
        newfp_str = (struct framephy_struct *)malloc(sizeof(struct framephy_struct));
        newfp_str->fpn = free_fpn;
        newfp_str->fp_next = caller->active_mswp->used_fp_list;
        newfp_str->owner = caller->mm;
        caller->active_mswp->used_fp_list = newfp_str;

        // enlist_framephy_node(frm_lst, victfpn);
        struct framephy_struct *newvicfp_str = (struct framephy_struct *)malloc(sizeof(struct framephy_struct));
        newvicfp_str->fpn = victfpn;
        newvicfp_str->fp_next = *frm_lst;
        newvicfp_str->owner = caller->mm;
        *frm_lst = newvicfp_str;
      }
    }
  }
  return 0;
...
int print_pgtbl(struct pcb_t *caller, uint32_t start, uint32_t end)
{
  ...
  printf("print_pgtbl of pid %d: %d - %d", caller->pid, start, end);
  if (caller == NULL)
  {
    printf("NULL caller\n");
    return -1;
